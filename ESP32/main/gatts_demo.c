...
/****************************************************************************
 *
 * This file configures and enables the ESP32 to act as a BLE GATT server 
 * using the ESP-IDF gatts_server example. To communicate tactile information
 * it also must configure and enable both the ADC and UART peripherals.
 *
 ****************************************************************************/
...
#define TEST_DEVICE_NAME "robotesp32_2"
...
void uart_init() {
  uart_config_t uart_cfg = {.baud_rate = UART_BAUDRATE,
...
#define ADC_INPUT1 ADC1_CHANNEL_6 // GPIO 34
#define ADC_INPUT2 ADC1_CHANNEL_7 // GPIO 35
void adc_init() {
  adc1_config_width(ADC_BITWIDTH_12);
  adc1_config_channel_atten(ADC_INPUT1, ADC_ATTEN_DB_12);
  adc1_config_channel_atten(ADC_INPUT2, ADC_ATTEN_DB_12);
}
...
static uint8_t adv_service_uuid128[16] = {
    0xfb, 0x34, 0x9b, 0x5f, // Bluetooth Base UUID
    0x80, 0x00, 0x00, 0x80, 0x00, 0x10,
    0x00, 0x00, 0xEE, 0x00, 0x00, 0x00 // Custom UUID (EE00)
};
...
static void gatts_event_handler(esp_gatts_cb_event_t event,
  ...
  if (event == ESP_GATTS_REG_EVT) {
    ...
    char received_str[len + 1];
    memcpy(received_str, data, len);
    received_str[len] = '\n'; // Null-terminate

    // printf("Received Data: %s\n", received_str);

    // Extract the first integer from the received string
    int received_number = 0;
    sscanf(received_str, "%d", &received_number);
    // printf("Extracted Number: %d\n", received_number);

    uart_write_bytes(UART_NUM, received_str, len + 1);
...
void adc_task(void *arg) {
  while (1) {
    ...
    char obj_flag_f[] = "False\n";
    char obj_flag_t[] = "True\n";
    for (int i = 0; i < 30; i++) {
      int fsr1 = adc1_get_raw(ADC_INPUT1);
      if (fsr1 <= 3000) {
        uart_write_bytes(UART_NUM, obj_flag_t, strlen(obj_flag_t));
        break;
      } else {
        uart_write_bytes(UART_NUM, obj_flag_f, strlen(obj_flag_f));
      }
